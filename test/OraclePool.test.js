...
const MockOraclePool = contract.fromArtifact('MockOraclePool')
...
describe('OraclePool Implementation', () => {
    it('should only allow the DAO to call capture', async () => {
        expectRevert(capture({from: unauthorizedUser}), "OraclePool: Not DAO")
        await capture({from: mockDaoAddress})
    })

    it('should only allow the DAO to call `distributeToPools`', async () => {
        expectRevert(distributeToPools({from: unauthorizedUser, amount: token(100)}), "OraclePool: Not DAO")
        await distributeToPools({from: mockDaoAddress, amount: token(100)})
    })
    ...
    it('should only allow the DAO or Treasury to call addOraclePoolPair, removeOraclePoolPair', async () => {
        expectRevert(addOraclePoolPair({oracle: this.mockOracle0.address, pool: this.mockPool0.address, from: unauthorizedUser}),
            "OraclePool: Not Treasury or DAO"
        )
    ...
    it('should not allow the addition of duplicate oracles', async () => {
        ...
        expectRevert(addOraclePoolPair(pairWithDuplicateOracle),
            'OraclePool: This oracle already exists'
        )
    })
    ...
    it('should not allow the addition of duplicate pools', async () => {
        ...
        expectRevert(addOraclePoolPair(pairWithDuplicatePool),
            'OraclePool: This pool already exists'
        )
    })
    ...
    it('should allow the removal of oracle/pool pairs', async () => {
        ...
        expectRevert(removeOraclePoolPair({index: 2}), 'OraclePool: This pair does not exist')
        expect(await getOraclePoolPairs()).to.be.deep.equal([pair3, pair2])
        ...
        expectRevert(removeOraclePoolPair({index: 0}), 'OraclePool: This pair does not exist')
        expect(await getOraclePoolPairs()).to.be.deep.equal([])
    })
    ...
    it('should distribute rewards to pools proportionally based on their USD value bonded', async () => {
        ...
        await distributeToPools({amount: token(100)})
        expect(await balanceOf(pair0.pool.address)).to.be.bignumber.equal(token(25))
        expect(await balanceOf(pair1.pool.address)).to.be.bignumber.equal(token(25))
        expect(await balanceOf(pair2.pool.address)).to.be.bignumber.equal(token(25))
        expect(await balanceOf(pair3.pool.address)).to.be.bignumber.equal(token(25))
    })

    const [mockDaoAddress, mockTreasuryAddress, unauthorizedUser] = accounts

    const balanceOf = (address) =>
        this.token.balanceOf(address, {from: mockDaoAddress})

    const lastCapture = async () => {
        const result = await this.OraclePool.lastCapture();
    ...
    const capture = async ({from}) => {
        await this.OraclePool.capture({from: from || mockDaoAddress})
        const result = await this.OraclePool.lastCapture()
        const ratio = result[0].value
        const valid = result[1]
        return {ratio, valid}
    ...
    const addOraclePoolPair = async ({oracle, pool, from}) => {
        await this.OraclePool.addOraclePoolPair(oracle, pool, {from: from || mockDaoAddress})
    }

    const removeOraclePoolPair = async ({index, from}) =>
        await this.OraclePool.removeOraclePoolPair(index, {from: from || mockDaoAddress})

    const oraclePoolPair = async (index) => {
        const pair = await this.OraclePool.oraclePoolPairs(index, {from: mockDaoAddress})
        return {oracle: pair.oracle, pool: pair.pool};
    }

    const getOraclePoolPairs = async () => {
        const pairs = await this.OraclePool.getOraclePoolPairs({from: mockDaoAddress})
    ...
    const getPoolWeights = async () =>
        (await this.OraclePool.getPoolWeights.call()).map(r => new BN(r.value))
    ...
    const indexOfOracle = async (oracleAddress) =>
        this.OraclePool.indexOfOracle(oracleAddress, {from: mockDaoAddress})
    ...
    const indexOfPool = async (poolAddress) =>
        this.OraclePool.indexOfPool(poolAddress, {from: mockDaoAddress})

    const distributeToPools = ({from, amount}) =>
        this.OraclePool.distributeToPools.sendTransaction(amount, {from: from || mockDaoAddress})

    const MAX_UINT256 = new BN(2).pow(new BN(256)).subn(1);

    const token = (amount) => new BN(amount).mul(new BN(1e18.toString()))
    ...
    beforeEach(async () => {
        ...
        this.OraclePool = await MockOraclePool.new(
            this.token.address,
            mockDaoAddress,
            mockTreasuryAddress,
        ...
        await this.token.addMinter(this.OraclePool.address, {from: mockDaoAddress})
    })
});
