...
contract HybridOracleBase is IHybridOracle {
    ...
    bool lastRatioValid;
    Decimal.D256 lastTokenPerGoldViaBackingAsset;
    ...
    constructor (
        ...
        lastTokenPerGoldViaBackingAsset = Decimal.one();
        _OraclePool = Constants.getOraclePoolAddress();
        setup();
    }

    function setOraclePoolAddress(address parentAddress) onlyDaoOrTreasury external {
        Require.that(parentAddress != address(0), _file, "Hybrid Oracle cannot be null");
        _OraclePool = parentAddress;
    }

    function setup() private {
        (address token0, address token1) = (_pair.token0(), _pair.token1());
        _index = _token == token0 ? 0 : 1;
        Require.that(_index == 0 || _token == token1, _file, "Token not found");
    }

    function capture() public onlyOraclePool returns (Decimal.D256 memory, bool) {
        if (_initialized) {
            return updateOracle();
        } else {
    ...
    function updateOracle() private returns (Decimal.D256 memory, bool) {
        ...
        lastTokenPerGoldViaBackingAsset = goldPriceInUsd.div(goldPrice());
    ...
    function lastCapture() public view returns (Decimal.D256 memory, bool) {
        return (lastTokenPerGoldViaBackingAsset, lastRatioValid);
    }
    ...
    function updatePrice() private returns (Decimal.D256 memory) {
        ...
        address backingAsset = _pair.token0() == _token ? _pair.token1() : _pair.token0();
    ...
    modifier onlyOraclePool() {
        Require.that(_OraclePool != address(0) && msg.sender == _OraclePool, _file, "Not Hybrid Oracle");
        _;
    }
